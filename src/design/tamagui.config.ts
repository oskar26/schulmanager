...
const tokens = createTokens({
  color: {
    ...
    ...Object.fromEntries(Object.entries(palette.blocks).map(([name, hex]) => [`block${name[0].toUpperCase()}${name.slice(1)}`, hex])),
    priorityUrgent: palette.priority.urgent,
    prioritySoon: palette.priority.soon,
    priorityOk: palette.priority.ok,
    ...
    ...Object.fromEntries(Object.entries(darkPalette.blocks).map(([name, hex]) => [`darkBlock${name[0].toUpperCase()}${name.slice(1)}`, hex])),
  },
  space: { 0: 0, 1: space.xs, 2: space.sm, 3: space.md, 4: space.lg, 5: space.xl, 6: space.xxl, 7: space.xxxl, true: space.md },
